...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    k_k = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + np.eye(3) * sensor_var) # gain: k_k 9 x 3 matrix
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_cov[k] = f_jac @ p_cov[k-1] @ f_jac.T + l_jac @ (Q * delta_t * delta_t) @ l_jac.T
...
#    if lidar_i < len(lidar.t):
        if abs(lidar.t[lidar_i] - imu_f.t[k]) <= 1e3:
